...
d3.json('Data_Outputs/network-vectors.json').then(data => {
    ...
    .domain(d3.extent(data.nodes.map(node => node.keyword)));
        ...
        link = d3.select('.links')
            ...
            .join(
                enter => enter.append('line')
                    ...
                    .attr("x2", d => d.target.x)
                    .attr("y2", d => d.target.y)
                    .attr('stroke', d => nodeColor(d.source['keyword']) )
                    .attr('stroke-width', d => edgeScale(d.degree) )
                    .attr('opacity', 0.6),
                update => update
                    .attr("x1", d => d.source.x)
                    .attr("y1", d => d.source.y)
        ...
        node = d3.select('.nodes')
            ...
            .join(
                enter => enter.append('circle')
                    ...
                    .attr('fill', (d) => nodeColor(d.keyword)),
                update => update
                    .attr("cx", d => d.x)
                    .attr("cy", d => d.y)
                    .attr('r', (d) => nodeScale(d.degree))
