...
d3.json('Data_Outputs/network-vectors.json').then(data => {
    // console.log(data);

    // let subset_links = data.links.map(d => Object.create(d)); 
    // let subset_nodes = data.nodes.map(d => Object.create(d));
    
    // subset_nodes = subset_nodes.filter( d => (d.meta_magazine === "Marsden Magazines") );

    // // Create list of subset_nodes; filter links if source and target are in subset_nodes.
    // let nodes_id = Object.keys(subset_nodes).map((id) => [id, subset_nodes[id]]);
    // // let nodes_id = subset_nodes.map(d => d.id);
    // subset_links = subset_nodes.filter((d) => nodes_id.includes(d.source) && nodes_id.includes(d.target));

    // // Build first-step for focus/unfocus: adjlist + neigh()
    // subset_links.forEach(function(d) {
    //     adjlist.push(d.source + '-' + d.target);
    // });

    // let subset = Object.create(subset_nodes, subset_links);

    // console.log(subset_nodes);
...
function chart(subset) {
    const links = subset.links.map(d => Object.create(d));
    const nodes = subset.nodes.map(d => Object.create(d));
